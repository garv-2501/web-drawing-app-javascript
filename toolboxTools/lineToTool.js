function LineToTool() {
    ...
    this.populateOptions = function () {
        let optionsHTML = {
            penSizePrompt:
                "<label for='input' class='options-label'>Line Thickness:</label>",
            penSizeInput:
                "<form class='increase-decrease-input'>  <div class='value-button' id='line-decrease' value='Decrease Value'>-</div>  <input type='number' class='number-input' id='line-number-input' value='2'/>  <div class='value-button' id='line-increase' value='Increase Value'>+</div>  </form>",
        };
        select(".options").html(
        ...
        select("#line-increase").mouseClicked(function () {
            let value = parseInt(
                document.getElementById("line-number-input").value,
                10
            );
